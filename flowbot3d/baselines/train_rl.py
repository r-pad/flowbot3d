...
import numpy as np
from mani_skill_learn.env import ReplayMemory, save_eval_statistics
...
class EpisodicStatistics:
    ...
    def get_mean(self):
        ...
        return (
            np.sum(self.history_lens) / num_episode,
            np.sum(self.history_rewards) / num_episode,
        )
...
def train_rl(
    ...
    n_checkpoint=None,
    n_eval=None,
    init_replay_buffers=None,
    init_replay_with_split=None,
    eval_cfg=None,
    ...
    if init_replay_buffers is not None and init_replay_buffers != "":
        replay.restore(
            init_replay_buffers, replicate_init_buffer, num_trajs_per_demo_file
        )
    ...
    if init_replay_with_split is not None:
        ...
        if init_replay_with_split[1] is None:
            ...
            files = [
                str(_)
                for _ in Path(folder_root).glob("*.h5")
                if re.split("[_-]", _.name)[1] in model_split_file
                and env_cfg.env_name == _.name[:-3]
            ]
    ...
    if rollout is not None:
        ...
        if "obs" not in replay.memory:
            ...
            exit(0)
        logger.info(
            f'State dim: {get_shape(replay["obs"])}, action dim: {replay["actions"].shape[-1]}'
        )
    ...
    for iteration_id in itertools.count(1):
        ...
        if n_steps > 0:
            ...
            (
                print_dict["episode_length"],
                print_dict["episode_reward"],
            ) = episode_statistics.get_mean()
        else:
            # For offline RL
            tf_logs.reset()
            for i in range(n_updates):
        ...
        ETA = td_format(
            (datetime.now() - begin_time) * (total_steps / (steps - warm_steps) - 1)
        )
