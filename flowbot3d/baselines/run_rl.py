...
from mani_skill_learn.utils.meta import (
    Config,
    DictAction,
    collect_env,
    get_logger,
...
def parse_args():
    ...
    dir_agent.add_argument(
        "--no-agent-type", default=False, action="store_true", help="no agent type"
    )
    dir_agent.add_argument(
        "--agent-type-first",
        default=False,
    ...
    parser.add_argument(
        "--test-name",
        help="The name of the folder to save the test result",
        default=None,
    )
    ...
    parser.add_argument(
        "--auto-resume",
        help="Auto-resume the checkpoint under work dir, "
        "the default value is true when in evaluation mode",
        action="store_true",
    )
    ...
    group_gpus.add_argument(
        "--gpu-ids", default=None, type=int, nargs="+", help="ids of gpus to use"
    )
...
def main_mfrl_brl(cfg, args, rollout, evaluator, logger):
    ...
    from mani_skill_learn.methods.builder import BRL, MFRL, build_brl, build_mfrl
    from mani_skill_learn.utils.torch import load_checkpoint

    from part_embedding.baselines.train_rl import train_rl
    ...
    if (
        cfg.get("eval_cfg", None) is not None
        and (
            cfg.eval_cfg.get("num_procs", 1) > 1
            and not cfg.eval_cfg.get("synchronize", True)
        )
        or (
            cfg.get("rollout_cfg", None) is not None
            and cfg.rollout_cfg.get("num_procs", 1) > 1
            and not cfg.eval_cfg.get("synchronize", True)
        )
    ):
        agent.share_memory()
    ...
    if not (dist.is_available() and dist.is_initialized()):
        logger.info(
            "We do not use distributed training, but we support data parallel in torch"
        )
    assert args.local_rank == 0
    if len(args.gpu_ids) > 1:
        logger.warning(
            "Use Data parallel to train model! It may slow down the speed when the model is small."
        )
        agent.to_data_parallel(
            device_ids=args.gpu_ids, output_device=torch.cuda.current_device()
        )
...
def main():
    ...
    if args.clean_up:
        if (
            ...
            or (
                ...
                and os.path.commonprefix(args.resume_from) == cfg.work_dir
            )
        ):
            print(
                "The system will ignore the clean-up flag, "
    ...
    if args.num_gpus is not None and args.gpu_ids is not None:
        ...
        if env_info_dict["num_gpus"] > 0:
            logger.warning(
                f"We will use cpu to do training, although we have {num_gpus} gpus!"
            )
    if args.gpu_ids is not None:
    ...
    from mani_skill_learn.methods.builder import BRL, MFRL
