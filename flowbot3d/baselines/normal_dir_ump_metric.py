...
def vanilla_grasping_policy(
    ...
    # Phase 2: Back Up
        ...
        if phase_counter >= 10:
            ...
            phase_counter += 1
            # if phase_counter < 20:
            # action[0:3] = aux_T[:3, :3] @ np.array([-1, 0, 0])
        else:
            phase_counter += 1
            action[0] = 0
...
def normal_calc(env, env_name, model, top_k, id, animation, cam_frame=False):
    """For the initial grasping point selection"""
    obs = env.get_obs()
    ...
    if not cam_frame:
        ...
        cam_pos = env.cameras[1].sub_cameras[0].get_pose().p
        ...
        dist = cdist(
            0.5
            * (env.agent.get_ee_coords()[0] + env.agent.get_ee_coords()[1]).reshape(
                1, 3
            ),
            pcd_all[mask_1],
        )[:, 1]
        ...
        if animation:
            animation.add_trace(
                ...
                torch.as_tensor(
                    [
                        0.5
                        * (env.agent.get_ee_coords()[0] + env.agent.get_ee_coords()[1])
                    ]
                ),
...
if __name__ == "__main__":
    ...
    result_dir = os.path.join(
        os.getcwd(), "umpmetric_results_master", "normal_cam_align_dir"
    )
    ...
    if debug:
        ...
        # animation_module.save_gif("fail.gif")
        save_html = animation_module.animate()
        if save_html:
            save_html.write_html(os.path.join(result_dir, "debug_normal.html"))
    else:
        bad_doors = [
            "8930",
            "9003",
