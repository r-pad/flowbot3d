...
from flowbot3d.visualizations import FlowNetAnimation, UMPAnimation
...
class NormalPullDirectionDetector:
    ...
    def choose_pull_direction(self, obs) -> npt.NDArray[np.float32]:
        _, pull_vector = NormalPullDirectionDetector.normal_calc(
            obs, self.animation, self.cam_frame
        )
        return pull_vector

    @staticmethod
    def normal_calc(obs, animation, cam_frame=False):
        ee_coords = obs["ee_coords"]
