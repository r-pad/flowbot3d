...
from flowbot3d.grasping.agents.bc.bc_datagen_gt_flow_grasping import GLOBAL_PULL_VECTOR
...
def bc_grasping_policy(
    ...
    dr = None
    goto_p2 = False
    if env._scene.get_contacts():
        contacts = env._scene.get_contacts()
        for c in contacts:
            if (
                (
                    c.actor0.name == "panda_leftfinger"
                    and c.actor1.name != "panda_rightfinger"
                    and c.actor1.name != "ground"
                )
                or (
                    c.actor0.name == "panda_rightfinger"
                    and c.actor1.name != "panda_leftfinger"
                    and c.actor1.name != "ground"
                )
                or (
                    c.actor1.name == "panda_rightfinger"
                    and c.actor0.name != "panda_leftfinger"
                    and c.actor0.name != "ground"
                )
                or (
                    c.actor1.name == "panda_lefttfinger"
                    and c.actor0.name != "panda_rightfinger"
                    and c.actor0.name != "ground"
                )
            ):
                goto_p2 = True
...
def run(env_name, level, cfg, bc_model, save_video, door, ajar, result_dir, dagger):
    ...
    for i in range(150):
        ...
        action, phase, phase_counter, dr = bc_grasping_policy(
            env, bc_agent, phase, phase_counter
        )
...
if __name__ == "__main__":
    ...
    if "flow" in model_name:
        config = "/home/ManiSkill-Learn/configs/bc/mani_skill_point_cloud_transformer_flow.py"
    else:
        config = (
            "/home/ManiSkill-Learn/configs/bc/mani_skill_point_cloud_transformer.py"
        )

    if "dagger" in model_name:
        bc_model = "/home/ManiSkill-Learn/baselines_comparisons_results/{}/BC/models/model_10000.ckpt".format(
            model_name
        )
    else:
        bc_model = "/home/ManiSkill-Learn/baselines_comparisons_results/{}/BC/models/model_100000.ckpt".format(
            model_name
        )
    if debug:
    ...
    for num, i in enumerate(available_envs[start_ind:end_ind]):
        ...
        if dagger and os.path.isfile(
            ...
            trans_cmd = f"python tools/convert_demo_pcd.py --max-num-traj=-1 --env-name={i} --traj-name=/home/{os.getlogin()}/flowbot3d/umpnet_dagger_demo_traj/{i}/trajectory.h5 --output-name=./umpnet_dagger_demo_traj/{i}/traj_pcd.h5 --obs-mode pointcloud"
            os.system(trans_cmd)
            os.chdir(cwd)
            shutil.rmtree(
